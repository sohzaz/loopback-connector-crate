 ...
 */
const crate = require('cratejs');
const SqlConnector = require('loopback-connector').SqlConnector;
const util = require('util');
const async = require('async');
const debug = require('debug')('loopback:connector:crate');
...
Crate.prototype.create = function (model, data, callback) {
  ...
  Object.keys(data).forEach(k => {
    correctedData[snakeCase(k)] = data[k];
  })
  //Inserting a single row

  insertModel.data(correctedData)
...
Crate.prototype.all = function all(model, filter, callback) {
  ...
  if (filter.where) {
    Object.keys(filter.where).forEach(k => {
      where[snakeCase(k)] = filter.where[k];
    })
  } else {
    where = undefined;
  }
...
Crate.prototype.buildCluster = function (model) {
  var self = this;
  var sql = '';
  if (this._models[model].settings.table_options) {
    var options = this._models[model].settings.table_options.cluster_options || {};
    if (options && options.number_of_shards && Number.isInteger(options.number_of_shards)) {
      sql = 'clustered into ' +  options.number_of_shards + ' shards';
    }
  }
  return (sql);
...
Crate.prototype.createTable = function (model, cb) {
  ...
  self.query('CREATE TABLE ' + name + ' (\n  ' +
    self.propertiesSQL(model) + '\n) ' + self.optionsSQL(model) , cb);
  //});
};

Crate.prototype.optionsSQL = function (model) {
  var self = this;
  var sql = [];

  sql.push(self.buildCluster(model));
  sql.push(self.buildTableParams(model));
  return (sql.join('\n'))

}

Crate.prototype.buildTableParams = function (model) {
  var self = this;
  var sql = [];
  if (this._models[model].settings.table_options) {
    var options = this._models[model].settings.table_options.table_params || {};
  }
  for (var o in options) {
    var param = options[o];
    sql.push(o + '=' + JSON.stringify(param));
    console.log(o);
  }
  if (sql.length > 0) {
    console.log(sql)
...
Crate.prototype.buildIndexes = function(model) {
  var self = this;
  var indexClauses = [];
  ...
  for (var index in  indexes) {
    ...
    if (i.analyzer) {
      //TODO : add support for custom analyzer
      analyzer = 'WITH ( analyzer=' + i.analyzer + ')';
    }
    var indexedColumns = [];
    ...
    indexClauses.push( 'INDEX '+ indexName + ' ' + type + ' (' + columns + ') ' + analyzer);
  }
  return indexClauses;
}
